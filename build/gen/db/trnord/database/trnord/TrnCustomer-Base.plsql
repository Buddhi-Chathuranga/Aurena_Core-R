...
BEGIN
   ...
   Error_SYS.Fnd_Too_Many_Rows(Trn_Customer_API.lu_name_, NULL, methodname_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, customer_id_),
                            Formatted_Key___(company_id_, customer_id_));
   Error_SYS.Fnd_Record_Not_Exist(Trn_Customer_API.lu_name_);
...
BEGIN
   ...
   Error_SYS.Fnd_Record_Exist(Trn_Customer_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Fnd_Item_Format(Trn_Customer_API.lu_name_, name_, value_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.company_id, rec_.customer_id),
                            Formatted_Key___(rec_.company_id, rec_.customer_id));
   Error_SYS.Fnd_Record_Modified(Trn_Customer_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, customer_id_),
                            Formatted_Key___(company_id_, customer_id_));
   Error_SYS.Fnd_Record_Locked(Trn_Customer_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, customer_id_),
                            Formatted_Key___(company_id_, customer_id_));
   Error_SYS.Fnd_Record_Removed(Trn_Customer_API.lu_name_);
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_CUSTOMER_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(Trn_Customer_API.lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_CUSTOMER_PK') THEN
            Raise_Record_Exist___(newrec_);
         ELSE
            Raise_Constraint_Violated___(newrec_, constraint_);
...
BEGIN
   IF (company_id_ IS NULL OR customer_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
   SELECT company_id, customer_id, rowid, rowversion, rowkey,
          name, 
          credit_limit, 
          discount, 
          active
