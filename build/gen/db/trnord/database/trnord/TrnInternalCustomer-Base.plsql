...
TYPE Public_Rec IS RECORD
  (company_id                     TRN_CUSTOMER_TAB.company_id%TYPE,
   ...
   inventory_id                   TRN_CUSTOMER_TAB.inventory_id%TYPE);
...
BEGIN
   ...
   Error_SYS.Fnd_Too_Many_Rows(Trn_Internal_Customer_API.lu_name_, NULL, methodname_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, customer_id_),
                            Formatted_Key___(company_id_, customer_id_));
   Error_SYS.Fnd_Record_Not_Exist(Trn_Internal_Customer_API.lu_name_);
...
BEGIN
   ...
   Error_SYS.Fnd_Record_Exist(Trn_Internal_Customer_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Fnd_Item_Format(Trn_Internal_Customer_API.lu_name_, name_, value_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.company_id, rec_.customer_id),
                            Formatted_Key___(rec_.company_id, rec_.customer_id));
   Error_SYS.Fnd_Record_Modified(Trn_Internal_Customer_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, customer_id_),
                            Formatted_Key___(company_id_, customer_id_));
   Error_SYS.Fnd_Record_Locked(Trn_Internal_Customer_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(company_id_, customer_id_),
                            Formatted_Key___(company_id_, customer_id_));
   Error_SYS.Fnd_Record_Removed(Trn_Internal_Customer_API.lu_name_);
...
BEGIN
   BEGIN
      SELECT *
         ...
         AND   rowtype LIKE '%TrnInternalCustomer'
         FOR UPDATE;
      RETURN rec_;
   EXCEPTION
      WHEN no_data_found THEN
...
BEGIN
   ...
   END IF;
   IF (newrec_.company_id IS NOT NULL AND newrec_.branch_id IS NOT NULL AND newrec_.inventory_id IS NOT NULL)
   AND (indrec_.company_id OR indrec_.branch_id OR indrec_.inventory_id)
   AND (Validate_SYS.Is_Changed(oldrec_.company_id, newrec_.company_id)
     ...
     OR Validate_SYS.Is_Changed(oldrec_.inventory_id, newrec_.inventory_id)) THEN
      Trn_Inventory_API.Exist(newrec_.company_id, newrec_.branch_id, newrec_.inventory_id);
   END IF;
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_CUSTOMER_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(Trn_Internal_Customer_API.lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_CUSTOMER_PK') THEN
            Raise_Record_Exist___(newrec_);
         ELSE
            Raise_Constraint_Violated___(newrec_, constraint_);
...
FUNCTION Get_Inventory_Id (
   ...
   customer_id_ IN NUMBER ) RETURN NUMBER
IS
   temp_ trn_customer_tab.inventory_id%TYPE;
BEGIN
   IF (company_id_ IS NULL OR customer_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(company_id_, customer_id_, 'Get_Inventory_Id');
