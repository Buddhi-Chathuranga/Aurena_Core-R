...
--  (country_id                     TRN_COUNTRY_TAB.country_id%TYPE);

TYPE Public_Rec IS RECORD
  (country_id                     TRN_COUNTRY_TAB.country_id%TYPE,
   "rowid"                        rowid,
   rowversion                     TRN_COUNTRY_TAB.rowversion%TYPE,
   rowkey                         TRN_COUNTRY_TAB.rowkey%TYPE,
   country                        TRN_COUNTRY_TAB.country%TYPE);
...
FUNCTION Key_Message___ (
   country_id_ IN VARCHAR2 ) RETURN VARCHAR2 
IS
...
IS
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(country_id_),
                            Formatted_Key___(country_id_));
   Error_SYS.Fnd_Too_Many_Rows(Trn_Country_API.lu_name_, NULL, methodname_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(country_id_),
                            Formatted_Key___(country_id_));
   Error_SYS.Fnd_Record_Not_Exist(Trn_Country_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.country_id),
                            Formatted_Key___(rec_.country_id));
   Error_SYS.Fnd_Record_Exist(Trn_Country_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Fnd_Item_Format(Trn_Country_API.lu_name_, name_, value_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.country_id),
                            Formatted_Key___(rec_.country_id));
   Error_SYS.Fnd_Record_Modified(Trn_Country_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(country_id_),
                            Formatted_Key___(country_id_));
   Error_SYS.Fnd_Record_Locked(Trn_Country_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(country_id_),
                            Formatted_Key___(country_id_));
   Error_SYS.Fnd_Record_Removed(Trn_Country_API.lu_name_);
...
FUNCTION Lock_By_Id___ (
   objid_      IN VARCHAR2,
   objversion_ IN VARCHAR2 ) RETURN trn_country_tab%ROWTYPE
IS
   row_locked  EXCEPTION;
   PRAGMA      EXCEPTION_INIT(row_locked, -0054);
   rec_        trn_country_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  rec_
...
FUNCTION Lock_By_Keys___ (
   country_id_ IN VARCHAR2) RETURN trn_country_tab%ROWTYPE
IS
   rec_        trn_country_tab%ROWTYPE;
BEGIN
   BEGIN
   ...
   EXCEPTION
      WHEN no_data_found THEN
         Raise_Record_Removed___(country_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(country_id_, 'Lock_By_Keys___');
   END;
END Lock_By_Keys___;
...
BEGIN
   ...
   EXCEPTION
      WHEN row_locked THEN
         Raise_Record_Locked___(country_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(country_id_, 'Lock_By_Keys___');
      WHEN no_data_found THEN
         Raise_Record_Removed___(country_id_);
...
FUNCTION Get_Object_By_Id___ (
   objid_ IN VARCHAR2 ) RETURN trn_country_tab%ROWTYPE
IS
   lu_rec_ trn_country_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  lu_rec_
      FROM  trn_country_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN lu_rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, 'Get_Object_By_Keys___');
...
FUNCTION Check_Exist___ (
   country_id_ IN VARCHAR2 ) RETURN BOOLEAN
IS
   dummy_ NUMBER;
BEGIN
   IF (country_id_ IS NULL) THEN
      RETURN FALSE;
   END IF;
   SELECT 1
      INTO  dummy_
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN FALSE;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, 'Check_Exist___');
END Check_Exist___;
...
EXCEPTION
   WHEN no_data_found THEN
      ...
      objversion_ := NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, 'Get_Id_Version_By_Keys___');
...
PROCEDURE Unpack___ (
   newrec_   IN OUT trn_country_tab%ROWTYPE,
...
FUNCTION Pack___ (
   rec_ IN trn_country_tab%ROWTYPE ) RETURN VARCHAR2
IS
...
FUNCTION Public_To_Table___ (
   public_ IN Public_Rec ) RETURN trn_country_tab%ROWTYPE
IS
   rec_ trn_country_tab%ROWTYPE;
BEGIN
...
FUNCTION Table_To_Public___ (
   rec_ IN trn_country_tab%ROWTYPE ) RETURN Public_Rec
IS
   public_ Public_Rec;
BEGIN
...
FUNCTION Get_Indicator_Rec___ (
   rec_ IN trn_country_tab%ROWTYPE ) RETURN Indicator_Rec
IS
...
FUNCTION Get_Indicator_Rec___ (
   oldrec_ IN trn_country_tab%ROWTYPE,
   newrec_ IN trn_country_tab%ROWTYPE ) RETURN Indicator_Rec
IS
   indrec_ Indicator_Rec;
BEGIN
...
PROCEDURE Check_Insert___ (
   ...
   attr_   IN OUT VARCHAR2 )
IS
   oldrec_ trn_country_tab%ROWTYPE;
BEGIN
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_COUNTRY_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_COUNTRY_PK') THEN
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_COUNTRY_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(Trn_Country_API.lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_COUNTRY_PK') THEN
            Raise_Record_Exist___(newrec_);
         ELSE
            Raise_Constraint_Violated___(newrec_, constraint_);
...
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(newrec_.country_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(newrec_.country_id);
   END IF;
...
PROCEDURE Check_Delete___ (
   remrec_ IN trn_country_tab%ROWTYPE )
IS
   key_ VARCHAR2(2000);
BEGIN
   key_ := remrec_.country_id||'^';
...
PROCEDURE Remove___ (
   remrec_         IN OUT trn_country_tab%ROWTYPE,
   lock_mode_wait_ IN     BOOLEAN DEFAULT TRUE )
IS
   oldrec_     trn_country_tab%ROWTYPE;
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(remrec_.country_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(remrec_.country_id);
   END IF;
...
IS
   dummy_ trn_country_tab%ROWTYPE;
BEGIN
...
PROCEDURE Remove__ (
   ...
   action_     IN  VARCHAR2 )
IS
   remrec_ trn_country_tab%ROWTYPE;
BEGIN
   IF (action_ = 'CHECK') THEN
...
FUNCTION Get_Key_By_Rowkey (
   rowkey_ IN VARCHAR2 ) RETURN trn_country_tab%ROWTYPE
IS
   rec_ trn_country_tab%ROWTYPE;
BEGIN
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(rec_.country_id, 'Get_Key_By_Rowkey');
...
IS
BEGIN
   IF (NOT Check_Exist___(country_id_)) THEN
      Raise_Record_Not_Exist___(country_id_);
   END IF;
END Exist;
...
IS
BEGIN
   RETURN Check_Exist___(country_id_);
END Exists;
...
PROCEDURE Rowkey_Exist (
   rowkey_ IN VARCHAR2 )
IS
   country_id_ trn_country_tab.country_id%TYPE;
BEGIN
   IF (rowkey_ IS NULL) THEN
      RAISE no_data_found;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      Raise_Record_Not_Exist___(country_id_);
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, 'Rowkey_Exist___');
END Rowkey_Exist;
...
FUNCTION Get_Country (
   country_id_ IN VARCHAR2 ) RETURN VARCHAR2
IS
   temp_ trn_country_tab.country%TYPE;
BEGIN
   IF (country_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, 'Get_Country');
END Get_Country;
...
IS
   rowrec_ trn_country_tab%ROWTYPE;
BEGIN
   rowrec_ := Get_Key_By_Rowkey(rowkey_);
   RETURN Get(rowrec_.country_id);
END Get_By_Rowkey;
...
@UncheckedAccess
FUNCTION Get (
   country_id_ IN VARCHAR2 ) RETURN Public_Rec
IS
   temp_ Public_Rec;
BEGIN
   IF (country_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
   SELECT country_id, rowid, rowversion, rowkey,
          country
      INTO  temp_
      FROM  trn_country_tab
      WHERE country_id = country_id_;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, 'Get');
END Get;
...
FUNCTION Get_Objkey (
   country_id_ IN VARCHAR2 ) RETURN VARCHAR2
IS
   rowkey_ trn_country_tab.rowkey%TYPE;
BEGIN
   IF (country_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, 'Get_Objkey');
END Get_Objkey;
