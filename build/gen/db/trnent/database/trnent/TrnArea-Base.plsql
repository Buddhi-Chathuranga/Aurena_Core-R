...
--  (country_id                     TRN_AREA_TAB.country_id%TYPE,
--   area_id                        TRN_AREA_TAB.area_id%TYPE);

TYPE Public_Rec IS RECORD
  (country_id                     TRN_AREA_TAB.country_id%TYPE,
   area_id                        TRN_AREA_TAB.area_id%TYPE,
   "rowid"                        rowid,
   rowversion                     TRN_AREA_TAB.rowversion%TYPE,
   rowkey                         TRN_AREA_TAB.rowkey%TYPE,
   area                           TRN_AREA_TAB.area%TYPE);
...
FUNCTION Key_Message___ (
   country_id_ IN VARCHAR2,
   area_id_ IN VARCHAR2 ) RETURN VARCHAR2 
IS
   msg_ VARCHAR2(4000) := Message_SYS.Construct('ERROR_KEY');
BEGIN
   Message_SYS.Add_Attribute(msg_, 'COUNTRY_ID', country_id_);
   Message_SYS.Add_Attribute(msg_, 'AREA_ID', area_id_);
   RETURN msg_;
END Key_Message___;
...
FUNCTION Formatted_Key___ (
   ...
   area_id_ IN VARCHAR2 ) RETURN VARCHAR2
IS
   formatted_key_ VARCHAR2(4000) := Language_SYS.Translate_Item_Prompt_(lu_name_, 'COUNTRY_ID', Fnd_Session_API.Get_Language) || ': ' || country_id_ || ', ' ||
                                    Language_SYS.Translate_Item_Prompt_(lu_name_, 'AREA_ID', Fnd_Session_API.Get_Language) || ': ' || area_id_;
BEGIN
...
PROCEDURE Raise_Too_Many_Rows___ (
   country_id_ IN VARCHAR2,
   area_id_ IN VARCHAR2,
   methodname_ IN VARCHAR2 )
IS
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(country_id_, area_id_),
                            Formatted_Key___(country_id_, area_id_));
   Error_SYS.Fnd_Too_Many_Rows(Trn_Area_API.lu_name_, NULL, methodname_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(country_id_, area_id_),
                            Formatted_Key___(country_id_, area_id_));
   Error_SYS.Fnd_Record_Not_Exist(Trn_Area_API.lu_name_);
...
PROCEDURE Raise_Record_Exist___ (
   rec_ IN trn_area_tab%ROWTYPE )
IS
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.country_id, rec_.area_id),
                            Formatted_Key___(rec_.country_id, rec_.area_id));
   Error_SYS.Fnd_Record_Exist(Trn_Area_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Fnd_Item_Format(Trn_Area_API.lu_name_, name_, value_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(rec_.country_id, rec_.area_id),
                            Formatted_Key___(rec_.country_id, rec_.area_id));
   Error_SYS.Fnd_Record_Modified(Trn_Area_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(country_id_, area_id_),
                            Formatted_Key___(country_id_, area_id_));
   Error_SYS.Fnd_Record_Locked(Trn_Area_API.lu_name_);
...
BEGIN
   Error_SYS.Set_Key_Values(Key_Message___(country_id_, area_id_),
                            Formatted_Key___(country_id_, area_id_));
   Error_SYS.Fnd_Record_Removed(Trn_Area_API.lu_name_);
...
FUNCTION Lock_By_Id___ (
   objid_      IN VARCHAR2,
   objversion_ IN VARCHAR2 ) RETURN trn_area_tab%ROWTYPE
IS
   row_locked  EXCEPTION;
   PRAGMA      EXCEPTION_INIT(row_locked, -0054);
   rec_        trn_area_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  rec_
      FROM  trn_area_tab
...
FUNCTION Lock_By_Keys___ (
   country_id_ IN VARCHAR2,
   area_id_ IN VARCHAR2) RETURN trn_area_tab%ROWTYPE
IS
   rec_        trn_area_tab%ROWTYPE;
BEGIN
   BEGIN
      SELECT *
         ...
         AND   area_id = area_id_
         FOR UPDATE;
      RETURN rec_;
   EXCEPTION
      WHEN no_data_found THEN
         Raise_Record_Removed___(country_id_, area_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(country_id_, area_id_, 'Lock_By_Keys___');
   END;
END Lock_By_Keys___;
...
BEGIN
   ...
   EXCEPTION
      WHEN row_locked THEN
         Raise_Record_Locked___(country_id_, area_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(country_id_, area_id_, 'Lock_By_Keys___');
      WHEN no_data_found THEN
         Raise_Record_Removed___(country_id_, area_id_);
...
FUNCTION Get_Object_By_Id___ (
   objid_ IN VARCHAR2 ) RETURN trn_area_tab%ROWTYPE
IS
   lu_rec_ trn_area_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  lu_rec_
      FROM  trn_area_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN lu_rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, area_id_, 'Get_Object_By_Keys___');
...
FUNCTION Check_Exist___ (
   country_id_ IN VARCHAR2,
   area_id_ IN VARCHAR2 ) RETURN BOOLEAN
IS
   dummy_ NUMBER;
BEGIN
   IF (country_id_ IS NULL OR area_id_ IS NULL) THEN
      RETURN FALSE;
   END IF;
   SELECT 1
      INTO  dummy_
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN FALSE;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, area_id_, 'Check_Exist___');
END Check_Exist___;
...
EXCEPTION
   WHEN no_data_found THEN
      ...
      objversion_ := NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, area_id_, 'Get_Id_Version_By_Keys___');
...
PROCEDURE Unpack___ (
   newrec_   IN OUT trn_area_tab%ROWTYPE,
...
FUNCTION Pack___ (
   rec_ IN trn_area_tab%ROWTYPE ) RETURN VARCHAR2
IS
...
BEGIN
   ...
   IF (rec_.area IS NOT NULL) THEN
      Client_SYS.Add_To_Attr('AREA', rec_.area, attr_);
   END IF;
   RETURN attr_;
END Pack___;
...
FUNCTION Public_To_Table___ (
   public_ IN Public_Rec ) RETURN trn_area_tab%ROWTYPE
IS
   rec_ trn_area_tab%ROWTYPE;
BEGIN
...
FUNCTION Table_To_Public___ (
   rec_ IN trn_area_tab%ROWTYPE ) RETURN Public_Rec
IS
   public_ Public_Rec;
BEGIN
...
FUNCTION Get_Indicator_Rec___ (
   oldrec_ IN trn_area_tab%ROWTYPE,
   newrec_ IN trn_area_tab%ROWTYPE ) RETURN Indicator_Rec
IS
   indrec_ Indicator_Rec;
BEGIN
   ...
   indrec_.area := Validate_SYS.Is_Changed(oldrec_.area, newrec_.area);
   RETURN indrec_;
END Get_Indicator_Rec___;
...
IS
BEGIN
   IF (newrec_.country_id IS NOT NULL)
   AND (indrec_.country_id)
   AND (Validate_SYS.Is_Changed(oldrec_.country_id, newrec_.country_id)) THEN
      Trn_Country_API.Exist(newrec_.country_id);
   END IF;
   Error_SYS.Check_Not_Null(lu_name_, 'COUNTRY_ID', newrec_.country_id);
   Error_SYS.Check_Not_Null(lu_name_, 'AREA_ID', newrec_.area_id);
   Error_SYS.Check_Not_Null(lu_name_, 'AREA', newrec_.area);
END Check_Common___;
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_AREA_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_AREA_PK') THEN
...
IS
BEGIN
   Validate_SYS.Item_Update(lu_name_, 'COUNTRY_ID', indrec_.country_id);
   Validate_SYS.Item_Update(lu_name_, 'AREA_ID', indrec_.area_id);
   Check_Common___(oldrec_, newrec_, indrec_, attr_);
END Check_Update___;
...
BEGIN
   ...
   IF by_keys_ THEN
      UPDATE trn_area_tab
         SET ROW = newrec_
         WHERE country_id = newrec_.country_id
         AND   area_id = newrec_.area_id;
   ELSE
      UPDATE trn_area_tab
         SET ROW = newrec_
         WHERE rowid = objid_;
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'TRN_AREA_RK') THEN
            Error_SYS.Fnd_Rowkey_Exist(Trn_Area_API.lu_name_, newrec_.rowkey);
         ELSIF (constraint_ = 'TRN_AREA_PK') THEN
            Raise_Record_Exist___(newrec_);
         ELSE
            Raise_Constraint_Violated___(newrec_, constraint_);
...
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(newrec_.country_id, newrec_.area_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(newrec_.country_id, newrec_.area_id);
   END IF;
...
PROCEDURE Check_Delete___ (
   remrec_ IN trn_area_tab%ROWTYPE )
IS
   key_ VARCHAR2(2000);
BEGIN
   key_ := remrec_.country_id||'^'||remrec_.area_id||'^';
...
BEGIN
   ...
   ELSE
      DELETE
         FROM  trn_area_tab
         WHERE country_id = remrec_.country_id
         AND   area_id = remrec_.area_id;
   END IF;
END Delete___;
...
PROCEDURE Remove___ (
   remrec_         IN OUT trn_area_tab%ROWTYPE,
   lock_mode_wait_ IN     BOOLEAN DEFAULT TRUE )
IS
   oldrec_     trn_area_tab%ROWTYPE;
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(remrec_.country_id, remrec_.area_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(remrec_.country_id, remrec_.area_id);
   END IF;
...
FUNCTION Get_Key_By_Rowkey (
   rowkey_ IN VARCHAR2 ) RETURN trn_area_tab%ROWTYPE
IS
   rec_ trn_area_tab%ROWTYPE;
BEGIN
   ...
   SELECT country_id, area_id
      INTO  rec_.country_id, rec_.area_id
      FROM  trn_area_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(rec_.country_id, rec_.area_id, 'Get_Key_By_Rowkey');
...
IS
BEGIN
   IF (NOT Check_Exist___(country_id_, area_id_)) THEN
      Raise_Record_Not_Exist___(country_id_, area_id_);
   END IF;
END Exist;
...
IS
BEGIN
   RETURN Check_Exist___(country_id_, area_id_);
END Exists;
...
PROCEDURE Rowkey_Exist (
   rowkey_ IN VARCHAR2 )
IS
   country_id_ trn_area_tab.country_id%TYPE;
   area_id_ trn_area_tab.area_id%TYPE;
BEGIN
   IF (rowkey_ IS NULL) THEN
      RAISE no_data_found;
   END IF;
   SELECT country_id, area_id
   INTO  country_id_, area_id_
   FROM  trn_area_tab
   WHERE rowkey = rowkey_;
EXCEPTION
   WHEN no_data_found THEN
      Raise_Record_Not_Exist___(country_id_, area_id_);
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, area_id_, 'Rowkey_Exist___');
END Rowkey_Exist;


-- Get_Area
--   Fetches the Area attribute for a record.
@UncheckedAccess
FUNCTION Get_Area (
   country_id_ IN VARCHAR2,
   area_id_ IN VARCHAR2 ) RETURN VARCHAR2
IS
   temp_ trn_area_tab.area%TYPE;
BEGIN
   IF (country_id_ IS NULL OR area_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, area_id_, 'Get_Area');
END Get_Area;
...
IS
   rowrec_ trn_area_tab%ROWTYPE;
BEGIN
   rowrec_ := Get_Key_By_Rowkey(rowkey_);
   RETURN Get(rowrec_.country_id, rowrec_.area_id);
END Get_By_Rowkey;
...
FUNCTION Get (
   country_id_ IN VARCHAR2,
   area_id_ IN VARCHAR2 ) RETURN Public_Rec
IS
   temp_ Public_Rec;
BEGIN
   IF (country_id_ IS NULL OR area_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
   SELECT country_id, area_id, rowid, rowversion, rowkey,
          area
      INTO  temp_
      FROM  trn_area_tab
      WHERE country_id = country_id_
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, area_id_, 'Get');
END Get;
...
FUNCTION Get_Objkey (
   ...
   area_id_ IN VARCHAR2 ) RETURN VARCHAR2
IS
   rowkey_ trn_area_tab.rowkey%TYPE;
BEGIN
   IF (country_id_ IS NULL OR area_id_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(country_id_, area_id_, 'Get_Objkey');
END Get_Objkey;
